...
const db = mysql.createConnection({
  host: "localhost",
  user: "root",
  password: "123aws123", // Your MySQL password
  database: "russia_shop",
});
...
app.put("/api/admin/reviews/:id/approve", (req, res) => {
  ...
  db.query(
    ...
    (err) => {
      ...
      // Update product average rating
      db.query(
        'UPDATE products p SET avg_rating = (SELECT AVG(rating) FROM reviews WHERE product_id = p.id AND status = "approved"), total_reviews = (SELECT COUNT(*) FROM reviews WHERE product_id = p.id AND status = "approved") WHERE id = (SELECT product_id FROM reviews WHERE id = ?)',
        [id],
        (updateErr) => {
          if (updateErr) console.error("Error updating rating:", updateErr);
        },
